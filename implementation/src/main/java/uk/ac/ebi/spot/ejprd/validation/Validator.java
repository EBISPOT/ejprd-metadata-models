package uk.ac.ebi.spot.ejprd.validation;
...
//		ValidationAlgorithm validationAlgorithm = new RefineValidation(schema, (new RDF4J()).asGraph(dataModel));
		ValidationAlgorithm validationAlgorithm = new RecursiveValidation(schema, (new RDF4J()).asGraph(dataModel));
	...
	private static Model getData (String dataFileName) {
		...
		try {
			...
			dataModel  = Rio.parse(inputStream, dataFilePath.toUri().toURL().toString(), RDFFormat.TURTLE);
		} catch (Exception e) {
